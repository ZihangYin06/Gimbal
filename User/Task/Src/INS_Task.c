...
static void NormalizeQuaternion(float *q)
{
    float normInv = 1.0f / sqrtf(q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3]);
    q[0] *= normInv;
    q[1] *= normInv;
    q[2] *= normInv;
    q[3] *= normInv;
...
void INS_Task(void)
{
    ...
    if ((count % 1) == 0)
    {
        ...
        // 获取最终数据
        INS.Yaw = isfinite(QEKF_INS.Yaw) ? QEKF_INS.Yaw : INS.Yaw;
        INS.Pitch = isfinite(QEKF_INS.Pitch) ? QEKF_INS.Pitch : INS.Pitch;
        INS.Roll = isfinite(QEKF_INS.Roll) ? QEKF_INS.Roll : INS.Roll;
        INS.YawTotalAngle = isfinite(QEKF_INS.YawTotalAngle) ? QEKF_INS.YawTotalAngle : INS.YawTotalAngle;
    }
...
void QuaternionToEularAngle(float *q, float *Yaw, float *Pitch, float *Roll)
{
    ...
    *Pitch = atan2f(2.0f * (q[0] * q[1] + q[2] * q[3]), 2.0f * (q[0] * q[0] + q[3] * q[3]) - 1.0f) * 57.295779513f;
    {
        float sinRoll = 2.0f * (q[0] * q[2] - q[1] * q[3]);
