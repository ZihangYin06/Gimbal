...
{
    uint32_t *dwt_cnt = &ffc->DWT_CNT;
    ffc->dt = DWT_GetDeltaT((void *)dwt_cnt);
...
{
    uint32_t *dwt_cnt = &ldob->DWT_CNT;
    ldob->dt = DWT_GetDeltaT((void *)dwt_cnt);
...
float TD_Calculate(TD_t *td, float input)
{
    ...
    uint32_t *dwt_cnt = &td->DWT_CNT;

    td->dt = DWT_GetDeltaT((void *)dwt_cnt);
